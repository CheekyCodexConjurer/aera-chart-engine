...
- Requests are coalesced when `dataWindowMaxPending` is exceeded (`data.window.backpressure`).
- Coverage is inferred from primary series bounds unless explicitly overridden by the host (`setDataWindowCoverage`).
